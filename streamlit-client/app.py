...
async def main(st):
    ...
    if audio_value:
        ...
        audio_response_task = asyncio.create_task(receive_audio(s2s, st))

        if st.button("Close session"):
            await s2s.end_audio_input()
            await s2s.end_session()
            audio_response_task.cancel()
